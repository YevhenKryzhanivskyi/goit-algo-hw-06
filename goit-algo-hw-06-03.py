...
if __name__ == "__main__":
    ...
    metro_graph = add_edge_weights(metro_graph)
    ...
    # Алгоритм Дейкстри для всіх пар
    paths, lengths = dijkstra_all_pairs(metro_graph)
    print("\n=== Найкоротші шляхи між усіма вершинами (Дейкстра) ===")
    for source in paths:
        for target in paths[source]:
            print(
                f"{source} -> {target}: "
                f"шлях = {paths[source][target]}, "
                f"довжина = {lengths[source][target]}"
            )